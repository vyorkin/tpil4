...
-- Проекции: .val достаёт сам элемент, .property — его доказательство.
...
--
-- Nat.rec :
--   {f : Nat → Sort u} →         -- motive: тип результата зависит от числа
--   f zero →                      -- (1) что делать с нулём
--   ((n : Nat) → f n → f n.succ) → -- (2) что делать с n+1, зная f n
