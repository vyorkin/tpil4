...
-- Тактика replace заменяет гипотезу h новым утверждением,
-- которое нужно доказать с использованием старой h.
-- Синтаксис:
--   replace h : новый_тип := доказательство
--   replace h : новый_тип := by тактики
--   replace h with ⟨a, b⟩  -- деструктурирует и заменяет сразу
--
-- Чем replace отличается от have:
--   have h' : T := ...   — добавляет новую гипотезу, старая h остаётся
--   replace h : T := ... — удаляет старую h и добавляет новую h с тем же именем
--
-- Когда использовать replace вместо have:
--   — когда старая версия гипотезы больше не нужна и мешает читать контекст
--   — когда надо «уточнить» гипотезу (из h : P ∨ Q вытащить нужную ветку)
--   — когда надо деструктурировать гипотезу прямо на месте

-- Простой пример: заменяем Or-гипотезу её левой частью.
example (h : 1 = 1 ∨ 1 = 2) : 1 = 1 := by
  replace h : 1 = 1 := h.elim id (fun h' => absurd h' (by decide))
  exact h

-- Деструктурирующий replace: And-гипотезу разбиваем на части прямо на месте.
example (h : 2 = 2 ∧ 3 = 3) : 3 = 3 := by
  replace ⟨_, h2⟩ := h
  exact h2

-- Уточнение гипотезы через вычисление.
-- replace здесь заменяет h : n = 4 на доказанное следствие n % 2 = 0.
example (n : Nat) (h : n = 4) : n % 2 = 0 := by
  replace h : n % 2 = 0 := by rw [h]
  exact h

-- replace в цепочке рассуждений: уточняем гипотезу несколько раз подряд.
-- Сначала h : a = b, потом h заменяется на a = 0 через trans.
example (a b : Nat) (h : a = b) (h2 : b = 0) : a = 0 := by
  replace h : a = 0 := h.trans h2
  exact h
